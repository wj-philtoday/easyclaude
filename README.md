...
> **퍼블릭 베타** — Claude Code CLI(`claude`) 의 `--output-format stream-json` 출력을
> 웹 UI로 직접 받아 보여주는 가벼운 멀티-세션 워크벤치.

xterm.js 없이 동작하며, 단일 격리 HOME(overlay) 위에서 여러 claude 세션을 띄우고
대화·인증·MCP·플러그인·스킬을 한 곳에서 관리합니다.
